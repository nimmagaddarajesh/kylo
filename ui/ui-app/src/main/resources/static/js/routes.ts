import 'gsap' //otherwise TweenMax is not defined
...
//var app = angular.module("", ["ngRoute"]);
    configFn($ocLazyLoadProvider: any, $stateProvider: any, $urlRouterProvider: any) {
        ...
        // modules: ['kylo', 'kylo.common', 'kylo.services', 'kylo.feedmgr'],
        // asyncLoader: require,
        // debug: false
        // });
        ...
        $stateProvider
            .state('home', {
                ...
                lazyLoad: ($transition$) => {
                    ...
                    return import(/* webpackChunkName: "kylo-module" */ "./main/HomeController")
                        .then(mod => {
                            console.log('home mod', mod);
                            $ocLazyLoad.load(mod.default);
                        })
                        .catch(err => {
                            throw new Error("Failed to load main/HomeController, " + err);
                        });
                }
            });
        ...
        //     lazyLoad: ($transition$) => {
        //         const $ocLazyLoad = $transition$.injector().get("$ocLazyLoad");
        //         return import(/* webpackChunkName: "future.module" */ './future/future.module')
        //             .then(mod => {
        //                 $ocLazyLoad.load(mod.FUTURE_MODULE);
        //             })
        //             .catch(err => {
        ...
        $stateProvider.state({
            ...
            lazyLoad: ($transition$) => {
                const $ocLazyLoad = $transition$.injector().get("$ocLazyLoad");
                return import(/* webpackChunkName: "feedmgr.feeds.module" */ "./feed-mgr/feeds/feeds.module")
                    .then(mod => {
                        $ocLazyLoad.load(mod.default);
                    })
                    .catch(err => {
        ...
        //             return $ocLazyLoad.inject('main/AccessDeniedController');
        //         }]
        //     }
        // });
        //
